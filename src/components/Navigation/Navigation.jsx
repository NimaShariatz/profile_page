...
const Navigation = () => {
  ...
  const [mobile, setSize] = useState("35px");// usestate for re-rendering
  ...
  useEffect(() => {
    const handleResize = () => {
      //const newOpacity = window.innerWidth < 760 ? 1 : 0.4;// if less than 760, change to 1. else 0.4
      var newOpacity = "0.4"
      var newSize = "35px"
      if (window.innerWidth < 760){
        newSize = "5vw"
        newOpacity = "1"
        document.getElementById('navigation').style.paddingInline = '2.1vw';//no need for useState as its already re-rendering when setSize and setOpacity are called
        gsap.to("#navigation", { left: "5px", duration: 0.2 });
      }else{
        newSize = "30px"
        newOpacity = "0.4"
        document.getElementById('navigation').style.paddingInline = '15px';//no need for useState as its already re-rendering when setSize and setOpacity are called
        gsap.to("#navigation", { left: "25px", duration: 0.2 });
      }


      setSize(newSize);//re-render
      setOpacity(newOpacity);//re-render
      if (!isHovered) {
  ...
  useEffect(() => {
    const left_transition = window.innerWidth < 760 ? 5 : 25;
    
    gsap.fromTo("#navigation", { left: -100, ease:"power2.out" }, { left: left_transition, duration: 1.6 });
  ...
  return (
    <nav id='navigation' onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <a className='anchor' href="#" onMouseOver={() => changeBorderColor("#bedde8")} onMouseOut={() => changeBorderColor("rgba(205, 205, 205, 0.702)")}>
        <svg className="option_svg1" xmlns="http://www.w3.org/2000/svg" height={mobile} viewBox="0 -960 960 960" width={mobile}>
      ...
      <a className='anchor' href="#" onMouseOver={() => changeBorderColor("#e9adc4")} onMouseOut={() => changeBorderColor("rgba(205, 205, 205, 0.702)")}>
        <svg className="option_svg2" xmlns="http://www.w3.org/2000/svg" height={mobile}viewBox="0 -960 960 960" width={mobile}>
      ...
      <a className='anchor' href="#" onMouseOver={() => changeBorderColor("#edb563")} onMouseOut={() => changeBorderColor("rgba(205, 205, 205, 0.702)")}>
        <svg className="option_svg3" xmlns="http://www.w3.org/2000/svg" height={mobile} viewBox="0 -960 960 960" width={mobile}>
